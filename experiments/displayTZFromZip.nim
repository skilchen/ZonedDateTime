...
when isMainModule:
  var zonename: string
  var zipfile = paramStr(1)
  if paramCount() < 2:
    zonename = "UTC"
  else:
    zonename = paramStr(2)

  let tzinfo = initTZInfo(zonename, tzOlson)

  let data = readTZZip(zipfile, zonename)
  #when defined(showFile):
  when true:
    for tdata in data.transitionData:
      #echo zonename & " Version: ", repr(tdata.version)
      echo zonename & " " & $tdata.header
      when true:
  ...
  when defined(showTime):
    var n = localFromTime(epochTime(), tzinfo)
